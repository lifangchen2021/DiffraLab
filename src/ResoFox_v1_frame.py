...
def create_resofox_frame(parent):
    window = Frame(parent)
...
#    plot_window = None
    window.plot_window = None  # 變成這個 Frame 物件的屬性
...
#                      - math.cos(lattice_anguler_gamma/180*Pi))/math.sin(lattice_anguler_alpha/180*Pi)/ math.sin(lattice_anguler_beta/180*Pi)
    ...
    label121 = Label(window, text = "Selection of Curve Color (1:Blue, 2:Orange, 3:Green) =" , font=("Arial", 14))
...
#            linecolor = "g"
        ...
        # Plot the curve
        plt.plot(angle_intensity_x, total_angle_intensity, linecolor, linestyle=linestyle)
    ...
    #        f"FWHMₘᵢₙ={round(a_1_over_2_min, 4)}"
        ...
        plt.plot(angle_intensity_x, total_angle_intensity, color=linecolor, label=label, linestyle=linestyle)
...
#            plot_window.destroy()
        if window.plot_window is not None and window.plot_window.winfo_exists():
            window.plot_window.destroy()
        window.plot_window = Toplevel()
#        plot_window = Toplevel()
        window.plot_window.title("Simulated Diffraction Pattern")

        # add Fig
        canvas = FigureCanvasTkAgg(plt.gcf(), master=window.plot_window)
        canvas.draw()
        ...
        Button(window.plot_window, text="Save Image", command=save_figure).pack(pady=5) 
        ...
        # === add botton ===
        Button( window.plot_window, text="Export Data", command=save_data).pack(pady=3)
        
        def clear_figure():
            plt.clf()
            canvas.draw()

        Button( window.plot_window, text="Clear Plot", command=clear_figure).pack(pady=2)
